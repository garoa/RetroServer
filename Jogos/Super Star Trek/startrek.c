 ...
 * startrek.c
 *
 * Super Star Trek Classic (v1.0)
 * Retro Star Trek Game 
 * C Port Copyright (C) 1996  <Chris Nystrom>
 * 
 * Changes by DQ, 19/09/2018 to fix compile errors and warnings
 *
 *     - gets     replaced by my_gets    (deprecated)
 *     - getline  replaced by my_getline (colides with getline standard function)
 *     - register replaced by int        (anachronism)
 *     - q4, q5   not used               (in course_control)
 *     - x5       not used               (in photon_torpedoes)
 *
 ...
 * 
 * See http://www.cfi.org/pub/ccn/startrek for more info.
 *
...
intro(void)
{
  ...
  if (sTemp[0] == 'y' || sTemp[0] == 'Y')
    showfile("/usr/local/games/startrek.doc");
...
// gets is deprecated and no longer present in the standard library
void 
my_gets(string str)
{
    int c;
